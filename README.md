...
- **4th-Order Butterworth Bandpass Filter**: Polled raw PPG signals (25Hz) are passed through a digital bandpass filter ($0.5\text{ Hz} - 4.0\text{ Hz}$) to isolate blood pulse frequencies ($30 - 240\text{ BPM}$) and remove low-frequency physical movement noise.
...
- **Zero-Allocation Memory Pool**: A static recycler pool (`BioDataHolder`) acts as a memory cache for incoming sensor callbacks. It recycles memory structures to ensure $0.0\text{ ms}$ garbage collection pause times.
